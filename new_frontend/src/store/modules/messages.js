  ...
  actions: {
    initWs({ commit, dispatch }, payload) {
      ...
      const ws = new WebSocket(
        `${wsScheme}://localhost:8000/ws/project/${payload.pid}/comments/`
      );
      ws.onmessage = (event) => {
        console.log("Yayy");
    ...
    async loadMessages({ state, commit }, payload) {
      try {
        const response = await backend_client.get(
          `message/project/${payload.pid}/`
        );
    ...
    async setMember({ state, commit }, payload) {
      await backend_client
        .get(`project/${payload.pid}/`)
        .then((response) => {
          console.log(response.data);
